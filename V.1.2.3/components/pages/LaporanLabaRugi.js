...
window.PageLabaRugi = {
    setup() {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const allTrans = await db.transactions.where('date').between(start, end, true, true).toArray();
                const allExp = await db.expenses.where('date').between(start, end, true, true).toArray();
    ...
    <div class="p-4 flex flex-col gap-5 bg-white min-h-full">
        ...
        <div class="py-8 border-b border-gray-100 text-center">
            <span class="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] block mb-2">Net Profit</span>
            <h2 :class="stats.totalLabaBersih < 0 ? 'text-red-600' : 'text-gray-800'" class="text-4xl font-black tracking-tighter">
        ...
        <div class="grid grid-cols-1 gap-3">
            <div class="flex justify-between items-center p-5 bg-gray-50 rounded-[1.5rem] border border-gray-100 shadow-sm">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-green-100 text-green-600 rounded-xl flex items-center justify-center"><i class="ri-funds-box-line text-2xl"></i></div>
                    <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Omzet</span>
                </div>
                <span class="font-black text-gray-800">{{ formatR(stats.totalOmzet) }}</span>
            </div>
            <div class="flex justify-between items-center p-5 bg-gray-50 rounded-[1.5rem] border border-gray-100 shadow-sm">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-red-100 text-red-600 rounded-xl flex items-center justify-center"><i class="ri-hand-coin-line text-2xl"></i></div>
                    <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Modal & Biaya</span>
                </div>
