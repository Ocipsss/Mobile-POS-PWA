...
// v2.9 - Support Eceran cartId & Logic Sync
...
const PagePenjualan = {
    ...
    setup(props) {
        ...
        // PERBAIKAN: Gunakan cartId untuk membedakan Eceran vs Bungkus
        const updateQty = (cartId, n) => {
            const item = props.cart.find(i => i.cartId === cartId);
            if (item) {
                item.qty += n;
                if (item.qty <= 0) {
                    ...
                    <div v-for="item in cart" :key="item.cartId" 
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-1.5 bg-white p-1 rounded-lg border border-slate-200">
                                <button @click="updateQty(item.cartId, -1)" class="w-7 h-7 flex items-center justify-center rounded-md text-red-500 font-black active:scale-75 transition-all">-</button>
                                <span class="text-[11px] font-black text-slate-700 min-w-[20px] text-center">{{ item.qty }}</span>
                                <button @click="updateQty(item.cartId, 1)" class="w-7 h-7 flex items-center justify-center rounded-md text-blue-600 font-black active:scale-75 transition-all">+</button>
                            </div>
                        </div>
